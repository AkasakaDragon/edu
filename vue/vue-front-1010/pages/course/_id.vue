...
export default {
  methods: {
    //获取课程详细信息
    getCourseInfo() {
      courseApi.getCourseInfo(this.courseWebVo.courseId).then((resp) => {
        this.chapterVideoList = resp.data.data.chapterVideoList;
        this.courseWebVo = resp.data.data.courseWebVo;
        this.courseWebVo.courseId = resp.data.data.courseWebVo.id;
      });
    },
    initComment() {
      comment
        .getPageList(this.page, this.limit, this.courseWebVo.courseId)
        .then((response) => {
